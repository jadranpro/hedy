...
if sys.version_info.major < 3 or sys.version_info.minor < 6:
    print(
        "Hedy requires Python 3.6 or newer to run. However, your version of Python is",
        ".".join(
            [
...
def load_adventure_assignments_per_level(lang, level):
    ...
    if current_user(request)["username"]:
        ...
        for program in user_programs:
            ...
            program_key = (
                "level"
                if not program.get("adventure_name")
                else program["adventure_name"]
            )
        ...
        assignments.append(
            {
                ...
                else loaded_programs.get(short_name)["code"],
                "loaded_program_name": ""
                if not loaded_programs.get(short_name)
                else loaded_programs.get(short_name)["name"],
            }
        )
    ...
    assignments.append(
        {
            "short_name": "level",
            "loaded_program": ""
            if not loaded_programs.get("level")
            else loaded_programs.get("level")["code"],
            "loaded_program_name": ""
            if not loaded_programs.get("level")
            else loaded_programs.get("level")["name"],
        }
    )
    return assignments
...
parse_logger = jsonbin.MultiParseLogger(
    jsonbin.JsonBinLogger.from_env_vars(), jsonbin.S3ParseLogger.from_env_vars()
)
...
if os.getenv("PROXY_TO_TEST_HOST") and not os.getenv("IS_TEST_ENV"):
    ab_proxying.ABProxying(
        app, os.getenv("PROXY_TO_TEST_HOST"), app.config["SECRET_KEY"]
    )
...
def parse():
    ...
    querylog.log_value(
        level=level, lang=lang, session_id=session_id(), username=username
    )
            ...
            if E.args[0] == "Invalid Space":
                ...
                if "character_found" in E.arguments.keys():
                    E.arguments["character_found"] = hedy_errors.get(
                        E.arguments["character_found"], E.arguments["character_found"]
                    )
...
def adventure_page(adventure_name, level):
    ...
    if (
        re.sub(r"\?.+", "", request.url.split("/")[len(request.url.split("/")) - 1])
        == adventure_name
    ):
        ...
        desired_level = 0
        if user["username"]:
            existing_programs = db_get_many(
                "programs", {"username": user["username"]}, True
            )
            for program in existing_programs:
                if (
                    "adventure_name" in program
                    and program["adventure_name"] == adventure_name
                    and program["level"] > desired_level
                ):
        ...
        if desired_level == 0:
            for key in adventure["levels"].keys():
                if type_check(key, "int") and (
                    desired_level == 0 or desired_level > key
                ):
                    desired_level = key
        level = desired_level
...
def error():
    error_messages = TRANSLATIONS.get_translations(
        requested_lang(), "ClientErrorMessages"
    )
    response = make_response(
        render_template("error_messages.js", error_messages=json.dumps(error_messages))
    )
...
def delete_program(user, program_id):
    ...
    db_update(
        "users", {"username": user["username"], "program_count": program_count - 1}
    )
...
def translate_fromto(source, target):
    ...
    return render_template(
        "translate-fromto.html", source_lang=source, target_lang=target, files=files
    )
